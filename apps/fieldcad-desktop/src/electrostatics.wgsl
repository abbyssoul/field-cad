// Batched f32 electrostatics evaluator. The CPU f64 implementation in
// `fieldcad-electrostatics` is the correctness oracle for this kernel.

const COULOMB_CONSTANT: f32 = 8.9875517e9;
...
struct Source {
    // xyz = world position, w = charge in coulombs
    position_charge: vec4<f32>,
...
struct SampleOutput {
    // xyz = electric field, w = potential
...
fn evaluate(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    var electric_field = vec3<f32>(0.0);
    var potential = 0.0;
    for (var source_index = 0u; source_index < params.counts.x; source_index += 1u) {
        let source = sources[source_index];
        let charge = source.position_charge.w;
        if charge == 0.0 {
            continue;
        }

        let displacement = position - source.position_charge.xyz;
        ...
        if source.distribution_radius.x < 0.5 {
            ...
            field_contribution = COULOMB_CONSTANT * charge * displacement
                * inverse_distance_cubed;
            potential_contribution = COULOMB_CONSTANT * charge * inverse_distance;
        } else if distance < radius {
            let radius_squared = radius * radius;
            let radius_cubed = radius_squared * radius;
            field_contribution = COULOMB_CONSTANT * charge * displacement / radius_cubed;
            potential_contribution = COULOMB_CONSTANT * charge / (2.0 * radius)
        ...
        let field_is_nan = any(electric_field != electric_field);
        let field_is_infinite = any(abs(electric_field) > vec3<f32>(3.402823e38));
    ...
    outputs[index].field_potential = vec4<f32>(electric_field, potential);
